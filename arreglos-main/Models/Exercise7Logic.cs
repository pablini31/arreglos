...
namespace ArrayExercises.WinForms.Models
{
    public class Exercise7Logic
    {
        ...
        /// <summary>
        /// Genera datos de calificaciones aleatorios para 9 alumnos y 4 parciales
        /// </summary>
        /// <returns>Matriz 9x4 con valores aleatorios de calificaciones entre 0 y 10</returns>
        public double[,] GenerateRandomGradesData()
        {
            double[,] gradesData = new double[9, 4]; // 9 alumnos, 4 parciales

            for (int student = 0; student < 9; student++)
            {
                for (int partial = 0; partial < 4; partial++)
                {
                    // Generar valores aleatorios entre 0 y 10 con un decimal
                    gradesData[student, partial] = Math.Round(random.NextDouble() * 10, 1);
                }
            }
